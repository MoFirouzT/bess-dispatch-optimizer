...
**Gate:** `tests/golden/test_ingestion_guard.py` (6 oracles + fallback/passthrough/hard-stop/log-distinctness) and `tests/property/test_ingestion_guard_faults.py` (no corrupted series passes healthy; no false positive on legitimate varying negatives/zeros; transport→outage vs content→anomaly) — full suite **72 passed + 1 skipped (integration)**, ruff/format/lint-imports(4 KEPT)/docs-lint clean.

**Remaining for R1.5b before the R1 ship** (not yet done): (1) route the backtest/example fetch path through `guarded_fetch` to demonstrate the ADR-0013 provenance composition end-to-end on synthetic data; (2) hero visual #3 — the ingestion-guard fault-handling before/after strip (§17), exported to `docs/figures/` and embedded in the README; (3) glossary "Data reliability & MLOps" section + README known-limitations/DoD updates.
