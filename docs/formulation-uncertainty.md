...
**CQR (the default; [CQR over split conformal](decisions/cqr-over-split-conformal.md)).** Replace the point model with lower/upper quantile regressors $\hat q_{\alpha/2}, \hat q_{1-\alpha/2}$; conformalize on $\mathcal C$ with the signed score $E_i = \max\{\hat q_{\alpha/2}(x_i)-y_i,\ y_i-\hat q_{1-\alpha/2}(x_i)\}$ and its $(1-\alpha)$ quantile $\hat s$, giving $[\hat q_{\alpha/2}(x)-\hat s,\ \hat q_{1-\alpha/2}(x)+\hat s]$. Same marginal guarantee; width is now **input-adaptive**, which matters because day-ahead prices are heteroscedastic (volatile peaks, calm nights). **Two variants exist and this project runs both** ([R2.1g](specs/drift-robust-conformal.md)): the *symmetric* form written above applies one margin $\hat s$ to both bounds, while the *asymmetric* form fits a separate constant per side. Both carry the same marginal guarantee. The shipped forecaster runs the asymmetric form, because it is MAPIE's default and R2.1 did not override it; the R2.1g weighted construction runs the symmetric form, because that is what its coverage bound is stated for. The asymmetric form is the narrower of the two where measured. Which one ships is R2.1g's to settle; until it does, this sentence is what keeps the file honest about the difference.

**Gate (statistical, not a hand-solved oracle).** The claim is that empirical coverage under the R1.4 walk-forward lies within $\pm 0.05$ of nominal (so $0.9 \Rightarrow [0.85, 0.95]$). Because coverage is a *sampling statistic* whose indicators cluster within a day, that claim is tested by a **day-block bootstrap interval** rather than by a point estimate: the gate fails only when the whole interval falls outside the band, that is, only when the data can rule the claim out ([R2.1d](specs/forecaster-evaluation.md)). Intervals obey $\underline{\pi}_t \le \hat\mu_t \le \overline{\pi}_t$; features are strictly pre-gate-closure (no leakage). Coverage is gated alongside **sharpness** (pinball loss against a seasonal-naive baseline), since coverage alone is satisfiable by an arbitrarily wide interval. **Exchangeability** is the critical assumption; a price-distribution shift breaks it, which is exactly what the R2.1b drift monitor and the 7-day rolling recalibration exist to manage. Measured, not only asserted ([R2.1f](specs/interval-sharpness.md)): at one fixed placement on NL, pooled coverage falls monotonically with how hard the price level trends, from $0.897$ in 2024 to $0.791$ across the 2021 crisis ramp.

**Selection on sharpness ([R2.1f](specs/interval-sharpness.md)).** The learners' hyperparameters are not free parameters left at library defaults but are *chosen*, so the choice is part of what the shipped forecaster means. Conformal calibration makes that a search over width alone: whatever the quantile models do, the conformal step moves the bounds until marginal coverage lands near $1-\alpha$, so a worse base model yields a wider interval rather than an uncovered one. Writing $\bar w$ for mean interval width, $\hat c$ for pooled coverage and $\delta_h$ for the largest deviation of per-hour coverage from nominal, the shipped configuration is
...
**Drift-robust calibration ([R2.1g](specs/drift-robust-conformal.md)).** The guarantee above is conditional on exchangeability, which day-ahead prices violate, and R2.1f measured the cost: coverage falls monotonically with trend, 0.897 in 2024 to 0.791 across the 2021 ramp. Two constructions replace the assumption with something weaker, and each fixes a different half.

*Weighted (nonexchangeable) conformal.* Attach weights $w_i = \rho^{n+1-i}$, $\rho \in (0,1]$, to the calibration scores in time order, normalize as $\tilde w_i = w_i / (\sum_j w_j + 1)$ with $\tilde w_{n+1} = 1/(\sum_j w_j + 1)$, and take the margin as the weighted quantile of the scores **plus an atom at $+\infty$**:

$$ \hat s = Q_{1-\alpha}\Big( \sum_{i=1}^{n} \tilde w_i \delta_{E_i} + \tilde w_{n+1} \delta_{+\infty} \Big). $$

The atom is what keeps the bound finite-sample; when the level lands on it the margin is genuinely infinite, and the code returns that rather than the largest score. Coverage is then at least $1-\alpha$ minus a **coverage gap** $\sum_i \tilde w_i d_{\mathrm{TV}}(R(Z), R(Z^i))$, where $Z^i$ swaps calibration point $i$ with the test point. Two corollaries make the gap reportable: a changepoint $k$ points back costs at most $\rho^{k}$, and drift bounded by $d_{\mathrm{TV}}(Z_i, Z_{n+1}) \le \epsilon(n+1-i)$ costs at most $2\epsilon/(1-\rho)$. At $\rho = 1$ the weights are equal, $\hat s$ is the $\lceil(1-\alpha)(n+1)\rceil$-th smallest score, and the construction is the split conformal above **exactly**; the gap bound is then 1, which is the honest reading of what unweighted conformal promises off-exchangeability.

*Adaptive conformal inference (ACI).* Emit day $t$'s interval at a level $\alpha_t$ that moves with realized miscoverage $\mathrm{err}_t \in [0,1]$ (the day's miss rate, since 24 hourly outcomes settle together):

$$ \alpha_{t+1} = \alpha_t + \gamma(\alpha - \mathrm{err}_t), \qquad \gamma > 0. $$

Expanding the recursion telescopes to an identity that holds for **any** sequence, $\big|\tfrac1T\sum_t \mathrm{err}_t - \alpha\big| = |\alpha_{T+1} - \alpha_1| / (\gamma T)$. Bounding the numerator gives the published long-run guarantee $\le (\max\{\alpha_1, 1-\alpha_1\} + \gamma)/(\gamma T)$, with **no assumption on the data-generating distribution**. That bound rests on the iterate staying in $[-\gamma, 1+\gamma]$, which in turn rests on saturation: below 0 the interval is the whole line and cannot miss, above 1 it is empty and cannot hit. **Clamping the emitted level removes that feedback**, so a clamped run keeps the identity and loses the bound; the identity is therefore what is reported, beside the count of clamped days.

**Considered but out of scope:** conditional (per-$x$) coverage guarantees (conformal gives only marginal, and ACI only long-run); cross-conformal / jackknife+ (heavier, not needed at this data scale); the nonexchangeable full-conformal and jackknife+ variants; step-size-free adaptive variants (DtACI, AgACI).
