...
- **R2.7 (2026-07-28)**: window-selection protocol for the §R2.5 distributions. **No optimizer change and no new quantity**: VSS, FV, tail value and bid-curve value are computed by the same formulas. What changed is that the *window set* became part of the claim rather than an artifact of whichever series was passed in, on two counts. Each window's random draws are now a function of $(\text{seed}, w)$ alone, so restricting to a chosen set of delivery days is a filter over the full-sweep distribution rather than a fresh experiment; and an interval on a distribution's median resamples whole blocks of consecutive days, because windows inside a block share all but one training day, which the per-window sign test that R2.5 relied on ignored. §R2.5 gains the protocol; the studies are re-measured over 2022 to 2025 on NL and BE instead of one 2024 quarter.
- **R2.8 (2026-07-29)**: seed reproducibility of the §R2.5 distributions. **No new quantity and no optimizer change.** The §R2.7 window protocol fixes which days are scored but not the scenario draw, so the identical protocol on the identical days under a different seed gives a different median: measured at a spread of 4.85 EUR over ten seeds against a block-interval width of 15.11, on a headline of about +6. That width is a property of the estimator and is *not* covered by the block interval, which resamples values drawn under one seed. §R2.5 therefore reports the **mean over seeds of the per-seed median**, with the observed range beside it and the default-seed value named. The range is explicitly not a confidence interval, and never combined with the window interval.
