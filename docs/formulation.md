...
This file holds the canonical mathematics of the **deterministic** model:
the physics, the wear cost, the feasibility conditions it implies, and the dual quantities read off a solved dispatch.
Uncertainty (forecast, scenarios, the two-stage program, bid curves) is in [formulation-uncertainty.md](formulation-uncertainty.md);
measurement protocols are in [formulation-evaluation.md](formulation-evaluation.md).
Where the code and this file disagree, this file governs.
Pure engineering / data-reliability phases (R1.4c ingestion guard, R1.5 serving, R2.1b drift monitor) change no optimizer math and have no section here.
Sections resting on a published method name it; most are standard MILP technique and cite nothing (see [references.md](references.md)).

*Assumes:
the house notation in [Conventions](conventions.md) (grid-side power, per-unit SoC, `π / e / η / Δt`).
Battery and power-market terms are defined in the [glossary](glossary.md);
each section is self-contained, so read Conventions first.*
...
the per-phase sections below (R1.1, R1.2) and in [formulation-uncertainty.md](formulation-uncertainty.md) (R2.3, R2.6) are canonical and carry the derivations, rationale, and evolving detail; if the two ever disagree, the section governs.
Current through R2.6 (R2.4 adds explanation and R2.5 adds evaluation protocols; neither changes the optimizer).*
...
Grid-side power $p^{ch}_t,p^{dis}_t\ge 0$;
SoC $e_t\in[e_{\min},e_{\max}]$;
charge indicator $u_t\in\{0,1\}$;
degradation cost $D_t\ge 0$;
net export $g_t\equiv p^{dis}_t-p^{ch}_t$.
...
| (4) | $-R\le g_t-g_{t-1}\le R$ (for $t\ge 2$) | ramp on net power |
...
**Stochastic layer (R2.3).**
Optimize over a scenario set $\{(\pi^{(s)},p_s)\}_{s=1}^S$ instead of one price path.
A non-anticipative day-ahead commitment $g^{DA}$ (R1.1-feasible) and a per-scenario recourse dispatch $g^{(s)}$ (R1.1-feasible) are tied by a recourse budget
...
$\lambda=0$ is the risk-neutral recourse problem;
sweeping $\lambda$ traces the mean-CVaR frontier.
The program reduces to the deterministic MILP at $S=1$, and reports VSS $=\mathrm{RP}-\mathrm{EEV}$ and EVPI $=\mathrm{WS}-\mathrm{RP}$ with the ordering $\mathrm{EEV}\le\mathrm{RP}\le\mathrm{WS}$.

**Bid-curve first stage (R2.6).** Replace the single day-ahead commitment with one branch per scenario, $g^{DA,(s)}$, each R1.1-feasible on its own SoC trajectory, and constrain the branches within each hour $t$ so they read the clearing price alone:
...
The stochastic layer is built in **extensive form**:
the day-ahead block and the $S$ per-scenario recourse blocks, each a full copy of the R1.1 model, go to the solver as one program, coupled only by the recourse budget and the CVaR cuts.
Nothing is decomposed.
See [formulation-uncertainty.md §R2.3](formulation-uncertainty.md#r23-risk-aware-two-stage-dispatch--intraday-recourse-optimizer-delta).

---

## R1.1. Deterministic core

*Standard MILP modeling, no governing reference;
see [references.md: R1.1](references.md#r11-deterministic-milp-dispatch) for domain context.*
...
| $\bar P$ | rated power, $\max(\bar P^{ch},\bar P^{dis})$; used by the R2.3 recourse budget | MW |
...
- **Physics fidelity (deliberately shallow).**
    ...
    On a day-ahead horizon the price-forecast error dwarfs the battery-model error, so extra cell fidelity buys second-order revenue against a first-order uncertainty; the R2 forecaster / stochastic layer is where accuracy actually pays.
    The tempting refinements are also non-convex: SoC-dependent efficiency is bilinear and rainflow wear is path-dependent (both in the R1.2 out-of-scope list), so either trades a fast, provably-optimal solve for a slow, locally-optimal one.
    Cheap, convexity-preserving additions (a linear self-discharge decay in balance (1); a 2-to-3-segment PWL efficiency curve) wait until a real asset demands them.
- **Price-taker (reflexivity out of scope).**
    The price path $\pi_t$ is an exogenous parameter: dispatch $g_t$ optimizes against it and never feeds back into it. This holds for the study asset (2 MWh / 1 MW), far too small to move day-ahead clearing in a bidding zone that trades in the gigawatts.
    A day-ahead price is the clearing point of an auction, so what the model sets aside is the game producing it: it forecasts that outcome as a given series (R2.1). The assumption breaks for a participant large enough that its own bids move the price it is forecasting, where the honest model carries an endogenous price (a residual supply curve); that is the R3 price-impact work.
    Submitting a **bid curve** is a separate matter and not a market-power one: it handles the clearing price being unknown at gate closure, which is [R2.6](formulation-uncertainty.md#r26-price-contingent-day-ahead-bid-curves-optimizer-delta) over the R2.2 scenario set.
- **Sense.**
...
*Algebraic corollaries of R1.1: no new theory, no governing reference (see [references.md: R1.3](references.md#r13-pre-flight-validation)).*

This section adds **no constraints, variables, or objective terms**.
It records the closed-form feasibility test the validation layer ([specs/dispatch-core.md](specs/dispatch-core.md)) evaluates *before* the solver.
...
*Standard LP duality, no governing reference: the water value and no-trade band are corollaries of R1.1/R1.2 stationarity, not imported.
...
This section adds **no constraints, variables, or objective terms**.
It records the dual quantities the explainability layer ([specs/explainability.md](specs/explainability.md)) reads off the *solved* R1.1/R1.2 dispatch.
...
**$\mu$ belongs to the chosen optimum, not to the price path.** Where $V^\star$ has a kink in $e_0$ its subdifferential is an interval and every point of that interval is a valid marginal value; where the *primal* optimum is non-unique, two equally optimal dispatches report different endpoints of it. A worked instance: $\pi=[0,-1,-1,0,0,0]$ on a 0.75 MWh / 2 MW asset ($\eta=1$, $e_0=e^{\mathrm{tgt}}$ half full, $\Delta t=0.5$) empties at $t_1$ and is then paid to absorb one full charge, which it can take at $t_2$ **or** $t_3$ for the same objective; the plan that charges at $t_2$ reports $\mu_2=-1$ and the plan that idles there reports $\mu_2=0$. So a claim of the form "$\mu$ is invariant under a transformation that leaves the price path alone" holds **only where the transformation leaves the dispatch alone**, which is the same kink caveat the finite-difference check carries. The tie-break invariance of [the MILP dual re-solve rule](decisions/milp-dual-resolve-rule.md) detects the ambiguity only when it appears as a negative-priced idle period; a plan that trades through the kink cannot see it.
...
**Phases.** Each phase's own section carries the statement and its argument; this table records only what the phase did to the model.

| Phase | What it added |
| --- | --- |
| R1.1 | the deterministic core |
| R1.2 | linear wear cost $D_t = c^{\text{deg}}\tau_t$ subtracted from the objective, R1.1 constraints unchanged |
| R1.3 | pre-flight feasibility corollaries of R1.1 |
| R1.4 | backtest semantics: foresight ceiling, rolling value, greedy floor, leakage information set |
| R2.1 | probabilistic price forecast (split/CQR conformal intervals) |
| R2.2 | scenario generation and reduction, the discrete uncertainty set R2.3 optimizes over |
...
**Amendments**, in date order. Each names what changed; the reasoning lives in the section it amends.

- **Errata (2026-07-08)**: R1.4's ordering restated as $V^{\mathrm{greedy}} \le V^{\mathrm{roll}} \le V^\star$ with $0 \le V^{\mathrm{roll}}$ (the old display's $0 \le V^{\mathrm{greedy}}$ contradicted the greedy-can-lose-money note); the sanity-band coefficient corrected to $c=\eta^{rt}(\text{cycles/day})\cdot 365$ ($E_{\text{usable}}$ wrongly appeared on both sides); R2.1 notation reconciled to $[\underline{\pi}_t,\overline{\pi}_t]$ with margin $\hat s$. No model change.
- **R1.2 model change (2026-07-11)**: the degradation cost was regrounded. The earlier convex-PWL-of-throughput form was self-derived and matched no published source; it is replaced by the **linear DoD-stress** case of the cited cycle-based model (Xu 2018; Shi 2017 §II-C-1), which is $\Delta t$-invariant and asset-scale-invariant. Governing reference updated in [references.md](references.md).
- **Split (2026-07-26)**: the Release-2 sections moved to a companion file at the 600-line cap, no math changed. §R1.1's price-taker note was corrected at the same time to separate price *impact* (R3) from a *bid curve* (§R2.6), which one sentence had conflated.
- **R2.4 clarification (2026-07-26)**: $\mu$ belongs to the *chosen* optimum, not to the price path, so an invariance claim about $\mu$ holds only where the dispatch is also invariant. No model change; the scale-invariance property gained the condition and a golden oracle now pins a measured instance.
- **R2.1d (2026-07-28)**: walk-forward evaluation hardening, with no change to the coverage guarantee. Folds are now placed across the whole evaluation span on a fixed-length rolling training window, the $\pm 0.05$ tolerance is decided by a day-block bootstrap interval rather than a point estimate, and sharpness joins coverage as a gated axis.
- **R2.1e (2026-07-28)**: an optional **normalized target** for the R2.1 forecaster, with no new guarantee. Fitting on $(\pi_t - m_t)/s_t$ and inverting by a strictly increasing affine map inherits R2.1's marginal coverage; what it buys is conditional calibration, since width in price space becomes proportional to the recent scale. De-levelling is additive, never a log or ratio, because a material share of hours clear at or below zero.
- **Recut by subject (2026-07-28)**: no math changed and no section body was edited. The files are now [formulation.md](formulation.md) (§R1.1, §R1.2, §R1.3, §R2.4), [formulation-uncertainty.md](formulation-uncertainty.md) (§R2.1, §R2.2, §R2.3, §R2.6) and [formulation-evaluation.md](formulation-evaluation.md) (§R1.4, §R2.5). Section numbers keep their historical phase labels, so every cross-reference resolves to the same mathematics in a new home. `scripts/lint_docs.py` gained a check binding every `formulation*.md §R<n>.<m>` reference in the repository to a section that exists.
- **Notation and structure (2026-08-31)**: the CVaR VaR auxiliary is renamed $\eta \to \zeta$, which had collided with the house efficiencies, and §R2.3 gained a statement of how the two-stage program is actually solved (extensive form, one day-ahead block plus $S$ recourse blocks in a single MILP). No model change; the binary count is restated as one mutual-exclusion copy per block.
