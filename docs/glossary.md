...
*Gotcha:* a single-duration headline can misstate the general result, so duration is a reported axis ([storage duration as a reported axis](decisions/storage-duration-reported-axis.md)), not a fixed asset.
...
**Extensive form (deterministic equivalent)**: a multi-stage stochastic program written out as one flat program, with an explicit copy of the second-stage variables and constraints for every scenario. *Why here:* R2.3's two-stage dispatch is built this way, one day-ahead block plus $S$ recourse blocks in a single MILP handed to HiGHS ([formulation-uncertainty.md §R2.3](formulation-uncertainty.md#r23-risk-aware-two-stage-dispatch--intraday-recourse-optimizer-delta)). *Gotcha:* it grows linearly in $S$ and is the thing decomposition exists to avoid; it is the right choice only while the coupling between blocks stays thin, which R2.6's bid curve is the counterexample to.
...
**Water value**: the shadow price of the SoC-balance constraint, the marginal worth of one more MWh of stored energy at that period (€/MWh); equivalently ∂V*/∂e₀ for the first period. *Why here:* the number R2.4's `/explain` reports, so a trader sees *why* the battery charged, discharged, or held ([formulation.md: R2.4](formulation.md#r24-shadow-price-explainability-derived-no-optimizer-change)). *Gotcha:* it is flat while SoC is interior and steps only where SoC hits a bound, so one number explains a whole run; the term is borrowed from hydro-thermal scheduling (the value of water behind a dam), context only, not a governing source.

**No-trade band**: the price interval `[η_ch(μ - c_deg), (μ + c_deg)/η_dis]` around the water value μ within which the battery idles (charge below it, discharge above). *Why here:* the explanation of an idle period, e.g. sitting through a spike because the price fell inside the band. *Gotcha:* its width comes from round-trip loss and wear, not the price (at η_rt=1 with zero degradation it collapses to the point π=μ, exact indifference); a transaction cost κ widens it flat by κ on each side, which is the per-trade breakeven-slippage read-off.

**Fix-and-resolve**: recovering duals from a MILP by fixing the integer variables at their optimum and re-solving the resulting LP, whose duals are then defined. *Why here:* a MILP has no LP dual, so this is how R2.4 reads the water value off the solved dispatch ([the MILP dual re-solve rule](decisions/milp-dual-resolve-rule.md)). *Gotcha:* the optimal commitment is not unique at an idle period (both `u_t` values are optimal) and the arbitrary tie-break moves the reported dual; the shipped rule relaxes the exclusion caps at non-negative-priced idle periods, keeps the commitment at negative-priced ones, and asserts the re-solved objective equals the MILP's.
...
**Split conformal vs. CQR**: two conformal constructions: split conformal adds a constant-width band around a point model; conformalized quantile regression (CQR) conformalizes lower/upper quantile models for *input-adaptive* width. *Why here:* R2.1 defaults to CQR ([CQR over split conformal](decisions/cqr-over-split-conformal.md)) because day-ahead prices are heteroscedastic. *Gotcha:* both guarantee only *marginal* coverage, never conditional; MAPIE 1.x CQR needs three prefit quantile models, not one.
...
**Regime shift vs. model staleness**: two causes of rising forecast error: the market genuinely changed (a naive baseline degrades too) vs. this model decayed (its error creeps up while a naive baseline's does not). *Why here:* R2.1b classifies them rather than emitting an undifferentiated "drift detected", staleness-first ([drift classification precedence](decisions/drift-classification-precedence.md)). *Gotcha:* conflating them is the realistic failure; retraining fixes staleness but not a regime shift.

**Interval miscalibration (coverage drift)**: a third drift state: the point forecast still tracks and inputs are stable, but the conformal *intervals* under-cover (empirical coverage falls materially below nominal). *Why here:* the forecaster's product is calibrated intervals, and R2.2 samples scenarios from them, so a silent coverage collapse is its own failure mode ([drift classification precedence](decisions/drift-classification-precedence.md)), checked after regime and mapped to *recalibrate*, not retrain. *Gotcha:* coverage over a short window is noisy, so the flag is one-sided (under-coverage only) and guarded by a minimum sample count.
