...
One canonical edit is required. [`formulation-r2.md` §R2.1](../formulation-r2.md#r21-probabilistic-price-forecast-conformal-intervals-no-optimizer-change)
...
| Upper bound reason | The SDAC market time unit moved to 15 minutes (ADR-0006) | `validate_utc_index` requires a single regular frequency, so a window crossing the switchover raises rather than silently mis-lagging. Handling mixed resolution is out of scope here |
...
- [x] Re-measure and restate: R2.1 coverage (both methods), R2.1c fundamentals
      ...
      the R2.1 and R2.1c specs, `formulation-r2.md` §R2.1, and `docs/STATE.md` to
