...
1. **Per-fold coverage runs 0.617 to 1.000.** Marginal coverage is essentially
   ...
   [`formulation-r2.md` §R2.1](../formulation-r2.md#r21-probabilistic-price-forecast-conformal-intervals-no-optimizer-change)
   already lists under "considered but out of scope". R2.1d turned it from an
   acknowledged caveat into a measured number.
2. **The property CQR was chosen for has never been tested.** ADR-0014 selected
...
`formulation-r2.md` §R2.1 gains a short paragraph for the normalized target: the
...
- [x] **Baseline first.** Measure and record hour-of-day conditional coverage for
      ...
      phase has nothing to improve against, and ADR-0014's adaptive-width claim
...
`formulation-r2.md` §R2.1, is violated by the shipped CQR model, which fits three
