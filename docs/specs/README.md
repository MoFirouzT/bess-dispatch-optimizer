...
| Price forecaster | [price-forecaster](price-forecaster.md), [forecaster-evaluation](forecaster-evaluation.md), [target-normalization](target-normalization.md), [interval-sharpness](interval-sharpness.md), [drift-robust-conformal](drift-robust-conformal.md) | R2.1, R2.1b, R2.1c / R2.1d / R2.1e / R2.1f / R2.1g | `bess.forecaster` |
...
| --- | --- | --- | --- |
| [dual-narration](dual-narration.md) | R2.4b | Built, gate ran, **not adopted** | A prose account of a solved dispatch in which the model emits placeholders and never a number, verified against the R2.4 `Explanation` before it is served, with a deterministic fallback. The acceptance gate ran at its specified 50 instances on 2026-09-04 and rejected **22.0%** against a 5% bar, so `POST /explain/narrative` does not ship; the verifier, the fallback and the offline gates stay, and the rate is the finding |
...
| [R2.1g](drift-robust-conformal.md) | 2026-08-31 | Price forecaster | weighted (nonexchangeable) conformal and adaptive conformal inference | **The arms work, and the refit cadence is what gates them.** Both constructions were built and gated against their published theorems (Barber et al. 2023 Thm 2a; Gibbs and Candes 2021 Prop 4.1). Scored on 2019 to 2025 at a monthly refit, the composed arm adds **+0.146 worst-year coverage on NL and +0.150 on BE** against a +0.03 bar, at 5.3% and 5.7% width; ACI alone fails the 5% clamp gate at 8.4% and 9.4%, and weighting is what relieves it. **Adoption is deliberately not taken:** `refit_every_days` lives only in the evaluation harness, nothing in the serving path schedules a refit, and the same arm clamps 71.6% of days and fails outright at an annual one, so the cadence is this phase's precondition. Refitting monthly rather than annually is itself worth +0.18 coverage on 2022 and +0.34 on 2021, with a 35% narrower interval. The gate was **amended on 2026-09-03**: the recorded null of +0.019 came from a shorter span whose worst year was 2022, and it was 2021, a genuinely broken baseline at 0.712, that separated the arms. Also found that the shipped CQR interval is the asymmetric variant, not the symmetric one §R2.1 defines, and that clamping the ACI level removes the feedback its guarantee rests on |
