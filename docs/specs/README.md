...
| --- | --- | --- | --- |
| [dual-narration](dual-narration.md) | R2.4b | Approved, built, not adopted | A prose account of a solved dispatch in which the model emits placeholders and never a number, verified against the R2.4 `Explanation` before it is served, with a deterministic fallback. Built and gated offline; live rejection rate 5.3% at n=19 against a 5% bar and a specified n of 50, so adoption is undecided |
...
| [R2.1g](drift-robust-conformal.md) | 2026-08-31 | Price forecaster | weighted (nonexchangeable) conformal and adaptive conformal inference | **Null, and the confound that hid it is the real finding.** Both constructions were built and gated against their published theorems (Barber et al. 2023 Thm 2a; Gibbs and Candes 2021 Prop 4.1), then measured: at a monthly refit they add at most +0.019 worst-year coverage on NL and nothing on BE, against a +0.03 bar. A first run at an *annual* refit showed the baseline covering the 2022 crisis year at 0.689 and the arms repairing it to 0.864, all of which was protocol rather than market. **Refitting monthly rather than annually is worth +0.18 coverage and a 35% narrower interval**, so the decay R2.1f attributed to exchangeability failure is mostly model staleness at this cadence. Also found that the shipped CQR interval is the asymmetric variant, not the symmetric one §R2.1 defines, and that clamping the ACI level removes the feedback its guarantee rests on. 2021, the worst year, stayed unscored: the fetch was blocked by an ENTSO-E outage |
