...
6. **Phase id.** **Resolved: R2.6**, because this is a first-stage decision over the R2.2 scenario set with no new data layer, and it reads naturally after R2.5b. Consequence: the `planning/` roadmap's R3.4 becomes the price-impact phase rather than the bid-curve phase, a rename to make in `planning/`.
