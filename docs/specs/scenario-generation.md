...
3. **Reduction algorithm.** **Resolved: fast forward selection** (Heitsch-Römisch) primary with probability redistribution, **k-means** the compared baseline ([forward selection over k-means](../decisions/forward-selection-over-kmeans.md)); forward selection carries the distance guarantee, k-means is the pragmatic reference point (mirrors [CQR over split conformal](../decisions/cqr-over-split-conformal.md)'s CQR-vs-split).
