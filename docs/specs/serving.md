...
- [x] Move `greedy_window` (+ helpers) to `bess.optimizer.heuristics`; update `bess.backtest.baselines` to re-export/import from there; all R1.4 gates stay green.
...
- [x] `bess.api.service.dispatch(...)`: pre-flight → solve-within-budget → greedy fallback, per [the dispatch circuit breaker](../decisions/dispatch-circuit-breaker.md).
