"""Battery asset — config (``BatterySpec``) + the plugin that registers the R1.1
deterministic formulation onto a Pyomo model.

Formulation: ``docs/formulation.md`` § "R1.1 — Deterministic core". All power is
**grid-side**; efficiency appears **only** in the SoC balance, never in the
objective (the objective is owned by ``optimizer.core``).
"""
    ...
    def register(self, model: pyo.ConcreteModel, prices: Sequence[float], dt: float) -> None:
        ...
        if s.ramp is not None:
            ...
            def _ramp(m, t):
                ...
                return (-s.ramp, net_t - net_prev, s.ramp)
